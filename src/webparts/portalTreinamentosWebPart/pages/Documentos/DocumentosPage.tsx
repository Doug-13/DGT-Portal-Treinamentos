    ...
    {documentos.length === 0 ? (
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><th>Código</th><th>Documento</th><th>Categoria</th><th>Revisão</th><th>Status</th></tr></thead>
          <tbody>
            {documentos.map(item => (
              <tr key={String(item.id)}>
                <td>{item.codigo}</td><td>{item.documento}</td><td>{item.categoria}</td>
                <td>{item.revisao}</td><td>{item.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
