      ...
      {carregando ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr .9fr .9fr', gap: '16px', marginTop: '20px' }}>
            ...
            <article style={{ ...card, padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <strong style={{ fontSize: '17px' }}>🗓 Próximos eventos</strong>
                <span style={{ color: '#0867D7', fontWeight: 700, fontSize: '13px' }}>Ver todos</span>
              </div>
              {[['15','MAI','Treinamento de NR-12','14:00 - 16:00'],['20','MAI','Semana da Qualidade','08:00 - 12:00'],['28','MAI','Segurança em Foco','10:00 - 11:30']].map(item => (
                <div key={`${item[0]}-${item[2]}`} style={{ display: 'grid', gridTemplateColumns: '54px 1fr', gap: '12px', padding: '12px 0', borderBottom: '1px solid #EDF1F5' }}>
                  <div style={{ padding: '7px 5px', borderRadius: '8px', background: '#EFF7F7', textAlign: 'center' }}>
                    <strong style={{ display: 'block', fontSize: '20px' }}>{item[0]}</strong><small>{item[1]}</small>
                  </div>
                  <div><strong>{item[2]}</strong><small style={{ display: 'block', marginTop: '5px', color: '#64748B' }}>◷ {item[3]}</small></div>
                </div>
              ))}
            </article>
          </div>
